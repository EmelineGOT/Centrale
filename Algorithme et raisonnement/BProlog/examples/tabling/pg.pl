...
numbervars(X):-number_vars(X,0,_).
...
number_vars(T, N0, N) :- 
    atomic(T),!,
     N0=N.
number_vars([X|Xs], N0, N) :- !,
    number_vars(X, N0, N1),
    number_vars(Xs, N1, N).
number_vars(X, N0, N) :-
    functor(X, _, A),
    number_vars(0, A, X, N0, N).

number_vars(A, A, _, N0, N) :- !,
    N0=N.
number_vars(A0, A, X, N0, N) :-
    A1 is A0+1,
    arg(A1, X, X1),
    number_vars(X1, N0, N1),
    number_vars(A1, A, X, N1, N).
...
ground(X):-functor(X,_,N),ground(X,N).

ground(X,N):-N=:=0,!,true.
ground(X,N):-N>0,arg(N,X,A),ground(A),N1 is N-1,ground(X,N1).
