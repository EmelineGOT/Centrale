...
australia(AusOs):-
  % background of flag
  cgRectangle(Flag),  
  Flag^width #> 300,   Flag^width #< 800,
  cgColor(BackgroundColor),
  BackgroundColor^blue #= 255,

  Flag^color #= BackgroundColor,    
  Flag^width #= 2*Flag^height,

  uk(UnionJackComps),
  UnionJackComps=[UnionFrame|_],
  UnionFrame^width #= Flag^width//3,  
  UnionFrame^x #= Flag^x,
  UnionFrame^y #= Flag^y,

  Stars=[CStar,Alpha,Beta,Gamma,Delta,Epsilon],
  cgStar(Stars),
  cgSame(Stars,color,white),

  CStar^n #= 7,
  CStar^width #= Flag^height//3,
  CStar^centerX #= UnionFrame^centerX,
  CStar^centerY #= Flag^y+Flag^height*2/3,

  cgSame([Alpha,Beta,Gamma,Delta],n,7),
  cgSame([Alpha,Beta,Gamma,Delta],width),
  Alpha^width #= Flag^height//5,

  Epsilon^n #= 5,
  Epsilon^width #= Flag^width//14,

  Alpha^centerX #= Flag^x+Flag^width*3//4,
  Alpha^centerY #= Flag^y+Flag^height*2//9,

  Beta^centerX #= Alpha^centerX,
  Beta^centerY #= Flag^y+Flag^height*7//9,
    
  Epsilon^centerX #= Flag^x+Flag^width*4//5,
  Epsilon^centerY #= Flag^y+Flag^height*1//2,

  Gamma^centerX #= Flag^x+Flag^width*6//7,
  Gamma^centerY #= Flag^y+Flag^height*3//9,

  Delta^centerX #= Flag^x+Flag^width*4//7,
  Delta^centerY #= Flag^y+Flag^height*3//8,
...
/*
  cgSame(Os,window,Win),
  cgPack(Os),
  cgSame(UnionJackComps,window,Win),
  cgPack(UnionJackComps),
*/
%  UnionFrame^x #= UX,UnionFrame^y #= UY,
%  UnionFrame^width #= UW,UnionFrame^height #= UH,
%  cgResize(UnionJackComps,UW,UH),
%  cgMove(UnionJackComps,UX,UY),

  append([Flag|Stars],UnionJackComps,AusOs).
