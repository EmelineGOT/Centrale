...
H = zeros(256,400);
for k=1:400
    H(:,k)= compute_hist(A(:,:,k));
    % compute_single_hist calcule l'histogramme pour toute l'image et
...
[R,T] = reference_single_list(H,L);%extraction des références et des visages à tester.
i = 1;
k = 1;
...
for p_test = 1:40 % pour chaque personne test
    for i_test = 1:6 %pour chaqune des 6 images de la personne test
        ...
        for p_ref = 1:40 % on compare aux 4 reférences de toutes les personnes.
            for i_ref = 1:4
                MatchingScore(i,p_ref) = MatchingScore(i,p_ref) - chi_single_hist(R(:,p_ref,i_ref),T(:,p_test,i_test));
